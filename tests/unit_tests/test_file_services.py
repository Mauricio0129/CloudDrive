...
async def test_retrieve_content_from_nonexistent_folder_raises_400(storage_services, valid_user_data, user_services,
                                                                   valid_folder_data_no_parent, db_pool):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await storage_services.retrieve_folder_content(user_id, sort_by="last_interaction", order="ASC",
                                                       location="e8d52a78-7140-4d03-afd8-d0bc43cf2c9b")

    assert exc_info.value.status_code == 400

async def test_retrieve_content_from_folder_works(storage_services, valid_user_data, user_services,
                                                  valid_folder_data_no_parent, db_pool):
    """Test that retrieving content from a non-existent folder raises 400."""
    # Create user who will own the folders
    user_id = await user_services.register_new_user(valid_user_data.username, valid_user_data.email,
    ...
    data = await storage_services.retrieve_folder_content(user_id, sort_by="last_interaction", order="ASC",
                                                       location=parent_folder_id)
    assert data["files_and_folders"][0]["name"] == folder_name

async def test_rename_folder_at_root_succeeds(storage_services, valid_user_data, user_services,
                                              valid_folder_data_no_parent, db_pool):
    ...
    # Rename folder
    await storage_services.rename_folder(user_id, None, folder_id, "test_folder_updated")
...
async def test_rename_folder_at_root_fails_when_name_at_use_at_location(storage_services, valid_user_data, user_services,
                                              valid_folder_data_no_parent, db_pool):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await storage_services.rename_folder(user_id, None, folder_id,
                                             valid_folder_data_no_parent.folder_name)
    assert exc_info.value.status_code == 409

async def test_rename_unexisting_folder_raises_404(storage_services, valid_user_data, user_services,
                                              valid_folder_data_no_parent, db_pool):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await storage_services.rename_folder(user_id, None, "e8d52a78-7140-4d03-afd8-d0bc43cf2c9b",
                                             "test_folder_updated")
    assert exc_info.value.status_code == 404

async def test_verify_folder_ownership_returns_name_for_owner(storage_services, valid_user_data, user_services, valid_folder_data_no_parent, db_pool):
    ...
    folder_name = await storage_services.register_folder(valid_folder_data_no_parent.folder_name, valid_folder_data_no_parent.parent_folder_id, user_id)
    ...
    result = await storage_services.verify_folder_existence_ownership(user_id, folder_id)

    assert result == valid_folder_data_no_parent.folder_name

async def test_verify_folder_ownership_returns_false_for_wrong_user(storage_services, valid_user_data, user_services,
                                                                    valid_folder_data_no_parent, db_pool):
    ...
    result = await storage_services.verify_folder_existence_ownership(second_user_id, folder_id)

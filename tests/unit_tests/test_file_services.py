...
async def test_upload_new_file_returns_presigned_url(
    ...
    mock_method = Mock(return_value=mock_s3_response)
    file_services.aws_services.generate_presigned_upload_url = mock_method

    user_id = await user_services.register_new_user(
        valid_user_data.username, valid_user_data.email, valid_user_data.password
    )

    response = await file_services.upload_an_new_file(valid_file_upload, user_id)
...
async def test_upload_duplicate_file_raises_conflict_error(
    ...
    await file_services.upload_an_new_file(valid_file_upload, user_id)
...
async def test_replace_existing_file_returns_presigned_url(
    ...
    response = await file_services.replace_existing_file(larger_file_upload, user_id)
...
async def test_get_file_returns_download_url_for_owned_file(
    ...
    mock_method_upload = Mock(return_value=mock_s3_response)
    mock_method_download = Mock(return_value=mock_s3_download_response)

    file_services.aws_services.generate_presigned_upload_url = mock_method_upload
    file_services.aws_services.generate_presigned_download_url = mock_method_download
    ...
    response = await file_services.get_user_presigned_download_url(user_id, file_id)
...
async def test_get_nonexistent_file_raises_not_found_error(
    file_services, user_services, valid_user_data
):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await file_services.get_user_presigned_download_url(user_id, file_id)
...
async def test_get_file_owned_by_other_user_raises_not_found_error(
    ...
    with pytest.raises(HTTPException) as exc_info:
        await file_services.get_user_presigned_download_url(second_user_id, file_id)

    assert exc_info.value.status_code == 404


async def test_rename_file_succeeds_when_name_available(
    db_pool, file_services, user_services, valid_user_data, valid_file_upload
):
    ...
    await file_services.rename_file(user_id, file_id, "new_file_name_after_Test", None)

    async with db_pool.acquire() as conn:
        result = await conn.fetchrow("SELECT name FROM files WHERE id = $1", file_id)
...
async def test_rename_file_fails_when_name_taken(
    ...
    await file_services.upload_an_new_file(valid_file_upload, user_id)
    await file_services.upload_an_new_file(valid_file_upload2, user_id)

    # Get the second file's ID
    async with db_pool.acquire() as conn:
        result = await conn.fetchrow(
            "SELECT id FROM files WHERE name = $1", valid_file_upload2.file_name
        )
        file_id = str(result["id"])
    ...
    with pytest.raises(HTTPException) as exc_info:
        await file_services.rename_file(user_id, file_id, "photo", None)
...
async def test_rename_file_fails_when_user_not_owner(
    ...
    with pytest.raises(HTTPException) as exc_info:
        await file_services.rename_file(
            second_user_id, file_id, "any_filename_test", None
        )
...
async def test_upload_file_exceeds_storage_raises_403(
    ...
    with pytest.raises(HTTPException) as exc_info:
        await file_services.upload_an_new_file(fake_ultra_heavy_file, user_id)
...
async def test_keep_both_generates_numbered_filename(
    ...
    await file_services.upload_an_new_file(valid_file_upload, user_id)
    await file_services.keep_both_files(valid_file_replica, user_id)
...
async def test_upload_to_nonexistent_folder_raises_400(
    ...
    with pytest.raises(HTTPException) as exc_info:
        await file_services.upload_an_new_file(file_with_parent_folder, user_id)
...
async def test_rename_file_fails_when_ext_is_included_available_400(
    ...
    with pytest.raises(HTTPException) as exc_info:
        await file_services.rename_file(
            user_id, file_id, "new_file_name_after_Test.png", None
        )

...
async def test_register_folder_at_root_succeeds(
    folder_services, valid_user_data, user_services, valid_folder_data_no_parent
):
    ...
    folder_name = await folder_services.register_folder(
        ...
        valid_folder_data_no_parent.parent_folder_id,
        user_id,
    )
...
async def test_duplicate_folder_at_root_raises_409(
    folder_services, user_services, valid_user_data, valid_folder_data_no_parent
):
    ...
    await folder_services.register_folder(
        valid_folder_data_no_parent.folder_name,
        valid_folder_data_no_parent.parent_folder_id,
        user_id,
    )
...
async def test_register_folder_at_set_location_succeeds(
    folder_services,
    valid_user_data,
    user_services,
    ...
    child_folder = await folder_services.register_folder(
        valid_folder_data_no_parent.folder_name, parent_folder_id, user_id
    )

    assert child_folder == valid_folder_data_no_parent.folder_name


async def test_register_duplicate_folder_at_set_location_raises_409(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
...
async def test_retrieving_empty_user_content_works(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
    ...
    data = await folder_services.retrieve_folder_content(
        user_id, "last_interaction", "ASC"
    )
    print(data)
    assert data["files_and_folders"] == []


async def test_retrieving_user_content_works(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
...
async def test_retrieve_content_from_nonexistent_folder_raises_400(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
    ...
    with pytest.raises(HTTPException) as exc_info:
        await folder_services.retrieve_folder_content(
            user_id,
...
async def test_retrieve_content_from_folder_works(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
    ...
    data = await folder_services.retrieve_folder_content(
        user_id, sort_by="last_interaction", order="ASC", location=parent_folder_id
    )
    assert data["files_and_folders"][0]["name"] == folder_name


async def test_rename_folder_at_root_succeeds(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
...
async def test_rename_folder_at_root_fails_when_name_at_use_at_location(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
    ...
    with pytest.raises(HTTPException) as exc_info:
        await folder_services.rename_folder(
            user_id, None, folder_id, valid_folder_data_no_parent.folder_name
        )
    assert exc_info.value.status_code == 409


async def test_rename_unexisting_folder_raises_404(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
    ...
    with pytest.raises(HTTPException) as exc_info:
        await folder_services.rename_folder(
            user_id, None, "e8d52a78-7140-4d03-afd8-d0bc43cf2c9b", "test_folder_updated"
        )
    assert exc_info.value.status_code == 404


async def test_verify_folder_ownership_returns_name_for_owner(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
...
async def test_verify_folder_ownership_returns_false_for_wrong_user(
    folder_services,
    valid_user_data,
    user_services,
    valid_folder_data_no_parent,
    ...
    result = await folder_services.verify_folder_existence_ownership(
        second_user_id, folder_id
    )
